...
procurement = sa.Table(
    "procurement",
    metadata,
    sa.Column("numero_controle_pncp", sa.Text(), primary_key=True),
    sa.Column(
        "source_raw_response_id",
        sa.BigInteger(),
        sa.ForeignKey("bronze.raw_api_response.id", name="fk_silver_procurement_raw"),
        nullable=False,
    ),
    ...
    sa.Index("ix_silver_procurement_raw", "source_raw_response_id"),
    sa.Index("ix_silver_procurement_orgao", "orgao_cnpj"),
    sa.Index("ix_silver_procurement_updated", "data_atualizacao_global"),
    schema="silver",
)

rejected_record = sa.Table(
    "rejected_record",
    metadata,
    ...
    sa.Column(
        "source_raw_response_id",
        sa.BigInteger(),
        sa.ForeignKey("bronze.raw_api_response.id", name="fk_silver_rejected_raw"),
        nullable=False,
    ),
    sa.Column("source_record_index", sa.Integer(), nullable=False),
